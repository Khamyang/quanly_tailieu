                        ...
                        <table id="tb_quan_ly_nhan_vien" class="table table-striped table-bordered">
                            <thead class="bg-warning">
                                ...
                                <th width="1%" class="text-center bg-warning" style="position: sticky;left:0px;z-index:100;"><pre style="width: 30px;">STT</pre></th>
                                    <th width="10%" class="text-center bg-warning" style="position: sticky;left:70px;z-index:100">
                                ...
                                <tr id="tr_them_nhan_vien" hidden>
                                    <form action="" id="form_them_nhan_vien" method="post">
                                        ...
                                        <td class="bg-warning text-center" style="position: sticky;left:70px;z-index:100">
                                        ...
                                        <td>
                                            <select class="form-control" id="phong" style="width: 100%;" required>
                                                ...
                                                ?>
                                                    <option value="<?=$row_them_phong->Ten_Phong?>">
                            ...
                            <tbody>
                                <?php 
                                    $stt = 0;
                                        $sql ="SELECT us.*,ph.* FROM tb_user as us LEFT JOIN tb_phong as ph ON us.Ma_Phong = ph.Ma_Phong";
                                ...
                                <tr id="1">
                                    ...
                                    <td class="text-center bg-white" style="position: sticky;left:70px;z-index:100">
                                        <div class="btn-group" role="group" aria-label="Basic mixed styles example">
...
function func_them_nhan_vien() {
    ...
    if (ho_ten != "" && so_dien_thoai != "" && email != "" && gioi_tinh != "" && ma_phong != "" &&
        ...
        xhr.open('POST', 'backend/them_nhanvien.php', true);
...
function func_xoa(ma_user) {
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "backend/xoa_nhanvien.php", true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.responseType = "json";
    xhr.onload = () => {
        if (xhr.status === 200) {
...
function func_cap_nhap(ma_user) {
    ...
    if (sua_ho_ten != "" && sua_so_dien_thoai != "" && sua_email != "" && sua_gioi_tinh != "" && sua_ma_phong != "") {
        var xhr = new XMLHttpRequest();
        xhr.open('POST', 'backend/sua_nhanvien.php', true);
        xhr.setRequestHeader("Content-Type", "application/json");
        xhr.responseType = 'json';
        xhr.onload = () => {
            if (xhr.readyState === 4 && xhr.status === 200) {
